  ...
  template: `
    <div>
      ...
      <div class="flex items-start justify-between mb-6 gap-4">
        <div>
          <h1 class="font-[Montserrat] text-[22px] font-extrabold text-[#1A1A2E]">Verificadores</h1>
          <p class="text-[13px] text-[#6B7280] mt-0.5">Registro y gestión del personal verificador</p>
        </div>
        <button
          (click)="openCreate()"
      ...
      <div class="bg-white rounded-[10px] border border-[#E0E0E0] shadow-[0_2px_12px_rgba(0,51,153,0.08)]">
        <div class="px-5 py-4 border-b border-[#E0E0E0] flex items-center justify-between gap-3">
          <span class="font-[Montserrat] font-bold text-[14px] text-[#1A1A2E]">Lista de verificadores</span>
            ...
            <table class="w-full text-[13px] border-collapse">
              ...
              <tbody>
                @for (e of paginated(); track e.id) {
                  ...
                  <tr>
                    <td colspan="6" class="px-5 py-12 text-center text-[#6B7280] text-[13px]">No se encontraron verificadores.</td>
                  </tr>
                }
              </tbody>
            </table>
    ...
    @if (showModal()) {
      <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
        ...
        [attr.aria-label]="editingId() ? 'Editar verificador' : 'Nuevo verificador'"
        (click)="closeModal()"
      >
            ...
            <section formGroupName="person_data">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <input id="v-bd" formControlName="birth_date" type="date" [max]="today"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                  />
                </div>
                <div class="flex flex-col gap-1">
...
export class VerifiersComponent implements OnInit {
  ...
  protected readonly ROLE = 'verificador';
  ...
  errMsg(controlName: string, label: string): string {
    const control = this.form.get(controlName);
    return getErrorMessage(control, label);
  }

  // 🔹 Validar mismatch de contraseñas
  ...
  // 🔹 Validación nested
  isInvalidNested(group: string, controlName: string): boolean {
    const control = this.form.get(`${group}.${controlName}`);
    return !!(control?.invalid && control?.touched);
  }

  // 🔹 Mensaje nested
  errMsgNested(group: string, controlName: string, label: string): string {
    const control = this.form.get(`${group}.${controlName}`);
    return getErrorMessage(control, label);
  }
