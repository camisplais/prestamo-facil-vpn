  ...
  template: `
    <div class="space-y-5">
      <div>
        ...
        <p class="text-[13px] text-[#6B7280] mt-1">Todos los vales y pre-vales registrados en el sistema</p>
      </div>
      ...
      <div class="flex flex-wrap gap-3 items-center">
        <div class="relative flex-1 min-w-[200px]">
          ...
          <input [(ngModel)]="filters.buscar" (input)="buscar()" placeholder="ID o nombre del cliente..."
        ...
        <select [(ngModel)]="filters.status" (change)="buscar()"
          ...
          <option value="on_hold">Pre-vale</option>
          <option value="created">Creado</option>
          <option value="cashed">Cobrado</option>
          <option value="active">Activo</option>
          <option value="paid_off">Liquidado</option>
          ...
          <table class="w-full text-[13px] border-collapse">
            ...
            <tbody>
              ...
              @for (v of items(); track v.id) {
                <tr class="border-b border-[#F0F0F0] hover:bg-[#FAFBFF] cursor-pointer" (click)="verDetalle(v.id)">
                  <td class="px-[14px] py-[12px]"><span class="font-mono text-[12px] bg-[#F0F4FF] text-[#003399] px-2 py-0.5 rounded">#{{ v.id }}</span></td>
                  <td class="px-[14px] py-[12px] font-semibold text-[#1A1A2E]">{{ v.final_customer?.person_data?.name }} {{ v.final_customer?.person_data?.first_last_name }}</td>
                  <td class="px-[14px] py-[12px] text-[#6B7280]">{{ v.product?.name ?? '—' }}</td>
                  <td class="px-[14px] py-[12px] font-semibold text-[#1A1A2E]">{{ v.total_value | currency:'MXN':'symbol':'1.2-2' }}</td>
                  <td class="px-[14px] py-[12px]">
                    <span [class]="estadoClass(v.status)" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold">{{ estadoLabel(v.status) }}</span>
                  </td>
                  <td class="px-[14px] py-[12px] text-[#6B7280] whitespace-nowrap">{{ v.created_at | date:'dd/MM/yyyy' }}</td>
                  <td class="px-[14px] py-[12px]">
                    <button (click)="$event.stopPropagation(); verDetalle(v.id)" class="text-[12px] font-semibold text-[#003399] hover:underline bg-transparent border-0 cursor-pointer p-0">Ver →</button>
                  </td>
                </tr>
              }
...
export class AdminValesComponent implements OnInit {
  ...
  cols    = ['#', 'Cliente', 'Producto', 'Monto total', 'Estado', 'Fecha', ''];

  ngOnInit() { this.buscar(); }
  buscar(page = 1) {
    this.loading.set(true);
    this.svc.vales({ ...this.filters, page }).subscribe({
  ...
  verDetalle(id: number) { this.router.navigate(['/admin/vales', id]); }
