  ...
  template: `
    <div>
      ...
      <div class="flex items-start justify-between mb-6 gap-4">
        <div>
          <h1 class="font-[Montserrat] text-[22px] font-extrabold text-[#1A1A2E]">Coordinadores</h1>
          <p class="text-[13px] text-[#6B7280] mt-0.5">Registro y gestión de coordinadoras del sistema</p>
        </div>
        <button
          (click)="openCreate()"
      ...
      <div class="bg-white rounded-[10px] border border-[#E0E0E0] shadow-[0_2px_12px_rgba(0,51,153,0.08)]">
        <div class="px-5 py-4 border-b border-[#E0E0E0] flex items-center justify-between gap-3">
          <span class="font-[Montserrat] font-bold text-[14px] text-[#1A1A2E]">Lista de coordinadoras</span>
          <input type="search" placeholder="Buscar…" (input)="search($event)"
            ...
            aria-label="Buscar coordinadora"
          />
        </div>
            ...
            <table class="w-full text-[13px] border-collapse">
              ...
              <tbody>
                @for (e of paginated(); track e.id) {
                  ...
                  <tr>
                    <td colspan="6" class="px-5 py-12 text-center text-[#6B7280] text-[13px]">No se encontraron coordinadoras.</td>
                  </tr>
                }
              </tbody>
            </table>
    ...
    @if (showModal()) {
      <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
        ...
        [attr.aria-label]="editingId() ? 'Editar coordinadora' : 'Nueva coordinadora'"
        (click)="closeModal()"
      >
        ...
        >
          <div class="px-5 py-4 border-b border-[#E0E0E0] flex items-center justify-between sticky top-0 bg-white z-10">
            <span class="font-[Montserrat] font-bold text-[15px]">
              {{ editingId() ? 'Editar coordinadora' : 'Nueva coordinadora' }}
            </span>
            ...
            <section>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-email">Correo electrónico *</label>
                  <input id="c-email" formControlName="email" type="email" placeholder="correo@ejemplo.com"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="isInvalid('email')" autocomplete="off"
                  />
                  @if (isInvalid('email')) { <span class="text-[11px] text-[#E53935]">{{ errMsg('email', 'Correo electrónico') }}</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-sub">Sucursal *</label>
                  <select id="c-sub" formControlName="subsidiary_id"
                  ...
                  @if (isInvalid('subsidiary_id')) { <span class="text-[11px] text-[#E53935]">Selecciona una sucursal.</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-pwd">
                    Contraseña {{ editingId() ? '(dejar vacío para no cambiar)' : '*' }}
                  </label>
                  <input id="c-pwd" formControlName="password" type="password" placeholder="Mínimo 8 caracteres"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="isInvalid('password')" autocomplete="new-password"
                  />
                  @if (isInvalid('password')) { <span class="text-[11px] text-[#E53935]">{{ errMsg('password', 'Contraseña') }}</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-pwdc">Confirmar contraseña</label>
                  <input id="c-pwdc" formControlName="password_confirmation" type="password" placeholder="Repetir contraseña"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="pwdMismatch" autocomplete="new-password"
                  />
                  @if (pwdMismatch) { <span class="text-[11px] text-[#E53935]">Las contraseñas no coinciden.</span> }
                </div>
              </div>
            </section>
            ...
            <section formGroupName="person_data">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="flex flex-col gap-1">
                  ...
                  <input id="c-name" formControlName="name" type="text" placeholder="María"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="isInvalidNested('person_data','name')"
                  />
                  @if (isInvalidNested('person_data','name')) { <span class="text-[11px] text-[#E53935]">{{ errMsgNested('person_data','name','Nombre') }}</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-fn">Primer apellido *</label>
                  <input id="c-fn" formControlName="first_last_name" type="text" placeholder="García"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="isInvalidNested('person_data','first_last_name')"
                  />
                  @if (isInvalidNested('person_data','first_last_name')) { <span class="text-[11px] text-[#E53935]">{{ errMsgNested('person_data','first_last_name','Primer apellido') }}</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-sn">Segundo apellido</label>
                  <input id="c-sn" formControlName="second_last_name" type="text" placeholder="López"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                  />
                </div>
                <div class="flex flex-col gap-1">
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <input id="c-bd" formControlName="birth_date" type="date" [max]="today"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                  />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-curp">CURP</label>
                  <input id="c-curp" formControlName="curp" type="text" placeholder="18 caracteres" maxlength="18"
                  ...
                  />
                  @if (isInvalidNested('person_data','curp')) { <span class="text-[11px] text-[#E53935]">{{ errMsgNested('person_data','curp','CURP') }}</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-rfc">RFC</label>
                  <input id="c-rfc" formControlName="rfc" type="text" placeholder="12 o 13 caracteres" maxlength="13"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] uppercase outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="isInvalidNested('person_data','rfc')"
                  />
                  @if (isInvalidNested('person_data','rfc')) { <span class="text-[11px] text-[#E53935]">{{ errMsgNested('person_data','rfc','RFC') }}</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-tel">Tel. personal</label>
                  <input id="c-tel" formControlName="personal_phone_number" type="tel" placeholder="10 dígitos"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="isInvalidNested('person_data','personal_phone_number')"
                  />
                  @if (isInvalidNested('person_data','personal_phone_number')) { <span class="text-[11px] text-[#E53935]">{{ errMsgNested('person_data','personal_phone_number','Teléfono') }}</span> }
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[12px] font-semibold text-[#1A1A2E]" for="c-cel">Celular</label>
                  <input id="c-cel" formControlName="phone_number" type="tel" placeholder="10 dígitos"
                    class="border border-[#E0E0E0] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#003399] transition-colors"
                    [class.border-[#E53935]]="isInvalidNested('person_data','phone_number')"
                  />
                  @if (isInvalidNested('person_data','phone_number')) { <span class="text-[11px] text-[#E53935]">{{ errMsgNested('person_data','phone_number','Celular') }}</span> }
                </div>
              </div>
            </section>
...
export class CoordinatorsComponent implements OnInit {
  ...
  protected readonly ROLE = 'coordinador';
