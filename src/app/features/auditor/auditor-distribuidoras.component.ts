...
@Component({
  selector: 'app-auditor-distribuidoras',
  standalone: true,
  ...
  template: `
    <div class="space-y-5">
      <div>
        ...
        <p class="text-[13px] text-[#6B7280] mt-1">Historial de cambios en distribuidoras, categorías y clientes finales</p>
      </div>
            ...
            <tbody>
              ...
              @for (log of logs(); track log.id) {
                ...
                @if (detalleAbierto() === log.id) {
                  <tr class="bg-[#F8FAFD]">
                    <td colspan="5" class="px-[14px] py-[12px]">
...
export class AuditorDistribuidorasComponent implements OnInit {
  ...
  filters = { desde: '', hasta: '' };
  cols = ['Método', 'Tabla', 'Usuario', 'Fecha', 'Cambios'];

  ngOnInit() { this.buscar(); }

  buscar(page = 1) {
    this.loading.set(true);
    this.svc.distribuidoras({ ...this.filters, page }).subscribe({
  ...
  limpiar() { this.filters = { desde: '', hasta: '' }; this.buscar(); }
