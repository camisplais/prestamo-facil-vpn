...
@Component({
  ...
  template: `
    <div class="space-y-5">
      <div>
        <h1 class="font-[Montserrat] text-[22px] font-extrabold text-[#1A1A2E]">Pagos y relaciones</h1>
        <p class="text-[13px] text-[#6B7280] mt-1">Historial de cambios en préstamos, detalles y cuentas bancarias</p>
      </div>
              ...
              @for (log of logs(); track log.id) {
                <tr class="border-b border-[#F0F0F0] hover:bg-[#FAFBFF]">
                  ...
                  <td class="px-[14px] py-[12px] font-mono text-[12px] text-[#6B7280]">
                    {{ log.new_data?.amount ?? log.new_data?.current_credit ?? '—' | currency:'MXN':'symbol':'1.2-2' }}
                  </td>
...
export class AuditorPagosComponent implements OnInit {
  ...
  filters = { metodo: '', desde: '', hasta: '' };
  cols = ['Método', 'Tabla', 'Monto', 'Usuario', 'Fecha', 'Cambios'];

  ngOnInit() { this.buscar(); }

  buscar(page = 1) {
    this.loading.set(true);
    this.svc.pagos({ ...this.filters, page }).subscribe({
  ...
  limpiar() { this.filters = { metodo: '', desde: '', hasta: '' }; this.buscar(); }
