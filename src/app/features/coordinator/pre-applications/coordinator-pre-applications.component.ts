    ...
    <div>
      ...
      <div class="flex items-start justify-between mb-6 gap-4">
        <div>
          ...
          <p class="text-[13px] text-[#6B7280] mt-0.5">Gestión de candidatas a distribuidoras</p>
        </div>
        <a
          [routerLink]="newPath()"
          class="inline-flex items-center gap-1.5 px-[18px] py-[9px] rounded-lg text-[13px] font-semibold bg-[#003399] text-white hover:bg-[#002277] transition-colors no-underline flex-shrink-0"
        >
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-[10px] p-5 border border-[#E0E0E0] shadow-[0_2px_12px_rgba(0,51,153,0.08)] flex gap-4">
          <div class="w-11 h-11 rounded-[10px] bg-[#003399]/10 text-[#003399] flex items-center justify-center flex-shrink-0">
            <svg class="w-[22px] h-[22px]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
          </div>
          <div>
        ...
        <div class="bg-white rounded-[10px] p-5 border border-[#E0E0E0] shadow-[0_2px_12px_rgba(0,51,153,0.08)] flex gap-4">
          <div class="w-11 h-11 rounded-[10px] bg-[#FF8800]/10 text-[#FF8800] flex items-center justify-center flex-shrink-0">
            <svg class="w-[22px] h-[22px]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </div>
          <div>
              ...
              <tbody>
                @for (p of paginated(); track p.id) {
                  <tr class="border-b border-[#F0F0F0] hover:bg-[#F8FAFD] transition-colors">
                    ...
                    <td class="px-5 py-3 text-[#6B7280]">{{ p.person_data?.phone_number || '—' }}</td>
                    <td class="px-5 py-3">
                  ...
                  <tr>
                    <td colspan="6" class="px-5 py-12 text-center text-[#6B7280] text-[13px]">
                      No hay pre-solicitudes registradas.
                    </td>
                  </tr>
                }
              </tbody>
...
export class PreApplicationsComponent implements OnInit {
  private router = inject(Router);
  readonly basePath = () => {
    const url = this.router.url.split('?')[0];
    return url.includes('/admin/') ? '/admin/coordinador/pre-solicitudes' : '/coordinador/pre-solicitudes';
  };
  readonly newPath = () =>
    this.router.url.includes('/admin/') ? '/admin/coordinador/pre-solicitudes/nueva' : '/coordinador/pre-solicitudes/nueva';
  ...
  headers = ['Nombre', 'CURP', 'Teléfono', 'Estado', 'Fecha', 'Acciones'];
  ...
  statusLabel = (s: string) => ({ pending: 'Pendiente', verified: 'Verificada', approved: 'Aprobada', rejected: 'Rechazada' }[s] ?? s);
